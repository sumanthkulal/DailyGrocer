...
// REMOVED: import axios from 'axios'; (This was causing the conflict)
...
const AddProduct = () => {
    ...
    // Using the configured axios from your AppContext
    ...
    const onSubmitHandler = async (event) => {
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                if (files[i]) { // Safety check to ensure file exists
            ...
            if (data.success) {
                toast.success(data.message); // Fixed typo: meessage -> message
                setName('');
                setDescription('');
                setCategory('');
                setPrice('');
